...
#if defined(USE_EINK)
            const int iconSize = 8 * NodeListRenderer::getIconScale();
...
void UIRenderer::drawNavigationBar(OLEDDisplay *display, OLEDDisplayUiState *state)
{
    ...
    const int scale = NodeListRenderer::getIconScale();
    const int iconSize = 8 * scale;
    const int spacing = 4 * scale;
    const int bigOffset = (scale > 1) ? scale / 2 : 0;

    const size_t totalIcons = screen->indicatorIcons.size();
    if (totalIcons == 0)
        return;

    const int navPadding = 12 * scale; // padding per side
